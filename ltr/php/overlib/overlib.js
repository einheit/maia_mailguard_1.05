...
function parseTokens(pf, ar)
{
    ...
    for (i = 0; i < ar.length; i++) {
        if (mode < 0) {
            ...
            if (ar[i]==STICKY) { if (pf!='ol_') { eval(pf+'sticky=1');
            } continue; }
            ...
            if (ar[i]==FULLHTML) { if (pf!='ol_') { eval(pf+'fullhtml=1');
            } continue; }
            ...
            if (ar[i]==FUNCTION) { if (pf=='ol_') { if (typeof ar[i+1]!='number') { v=ar[++i]; ol_function=(typeof v=='function' ? v : null); }} else {fnMark = 0; v = null; if (typeof ar[i+1]!='number') { v = ar[++i];
...
function opt_NOCLOSE(unused)
{
    ...
    if (olNs4) {
        ...
        over.onmouseover = function () {
            hoveringSwitch = true; if (o3_timerid > 0) { clearTimeout(o3_timerid); o3_timerid =0; } }
    }
