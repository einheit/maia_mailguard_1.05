...
function update_mail_stats($user_id, $type)
{
    ...
    if (!empty($token)) {
        ...
        $sth = $dbh->prepare(
            "SELECT MIN(received_date) AS mindate, " .
        ...
        if ($row = $res->fetchrow()) {
            ...
            if ($res2->fetchrow()) {
                ...
                $insertsth = $dbh->prepare(
                    "INSERT INTO maia_stats (oldest_" . $type . "_date, " .
                                                  "newest_" . $type . "_date, " .
                                                  "lowest_" . $type . "_score, " .
                                                  "highest_" . $type . "_score, " .
                                                  "total_" . $type . "_score, " .
...
function record_mail_stats($euid, $mail_ids, $type)
{
    ...
    foreach ((array)$mail_ids as $mail_id) {
        $sth = $dbh->prepare(
            "SELECT received_date, size, score " .
...
function count_total_mail($user_id)
{
    ...
    $sth = $dbh->prepare(
        "SELECT enable_false_negative_management, " .
...
function lowest_item_score($user_id, $type)
{
    ...
    if ($user_id > 0) {
        $sth = $dbh->prepare(
            "SELECT lowest_" . $type . "_score AS score FROM maia_stats " .
...
function highest_item_score($user_id, $type)
{
    ...
    if ($user_id > 0) {
        $sth = $dbh->prepare(
            "SELECT highest_" . $type . "_score AS score FROM maia_stats " .
...
function smallest_item_size($user_id, $type)
{
    ...
    if ($user_id > 0) {
        $sth = $dbh->prepare(
            "SELECT smallest_" . $type . "_size AS size FROM maia_stats WHERE user_id = ? AND smallest_" .
                  $type . "_size > 0"
        );
        $res = $sth->execute(array($user_id));
    } else {
...
function get_filter_stats($user_id)
{
    ...
    if ($user_id > 0) {
        $sth = $dbh->prepare(
            "SELECT total_ham_items AS total_ham, " .
        ...
        $sth = $dbh->prepare(
            "SELECT SUM(total_ham_items) AS total_ham, " .
