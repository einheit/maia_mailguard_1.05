...
function get_user_id($user_name, $email)
{
    ...
    if ($row = $res->fetchRow()) {
        ...
        if ($row = $res->fetchRow()) {
            ...
            $sth = $dbh->prepare(
                "UPDATE maia_users SET user_name = ? " .
                     "WHERE id = ?"
            );
...
function add_user($user_name, $email)
{
    ...
    $sth = $dbh->prepare(
        "INSERT INTO maia_users (user_name, reminders, charts, language, auto_whitelist, " .
...
function transfer_stats_to_user($old_owner_id, $new_owner_id)
{
    ...
    if ($row = $res->fetchRow()) {
        ...
        if ($row2 = $res2->fetchRow()) {
            ...
            $sthu = $dbh->prepare(
                "UPDATE maia_stats SET oldest_ham_date = ?, " .
                                            "newest_ham_date = ?, " .
                                            "smallest_ham_size = ?, " .
                                            "largest_ham_size = ?, " .
                                            "total_ham_size = ?, " .
            ...
            $sthi = $dbh->prepare(
                "INSERT INTO maia_stats (oldest_ham_date, " .
                                              "newest_ham_date, " .
                                              "smallest_ham_size, " .
                                              "largest_ham_size, " .
                                              "total_ham_size, " .
