...
{
    var k=i,v;
    if (k<ar.length) {if (ar[k]==FRAME) { v=ar[++k]; if(pf=='ol_'&&compatibleframe(v)) { ol_frame=v; } else { opt_FRAME(v);
    } return k; }
    }
    return-1;}
...
{
    if (olNs4&&typeof frameid.document.overDiv=='undefined') {return false;} else if (document.all&&typeof frameid.document.all["overDiv"]=='undefined') {return false;} else if (document.getElementById&&frameid.document.getElementById('overDiv')==null) {return false;}
    return true;}
function frmDepth(thisFrame,ofrm)
{
    var retVal='';
    for (var i=0; i<thisFrame.length; i++) {if (thisFrame[i].length>0) {retVal=frmDepth(thisFrame[i],ofrm);if (retVal=='') { continue;
